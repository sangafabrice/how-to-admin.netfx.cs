...
namespace cvmd2html
{
  static class Program
  {
    ...
    static int WaitForExit(int processId)
    {
      ...
      dynamic watcher = _.GetObject().ExecNotificationQuery(wmiQuery);
    ...
    static void RequestAdminPrivileges(string[] args)
    {
      ...
      var shell = new Shell();
      shell.ShellExecute(Path, String.Format(@"""{0}""", String.Join(@""" """, args)), Missing.Value, "runas", Constants.vbHidden);
      _.ReleaseComObject(ref shell);
      Quit(0);
    }
    ...
    static bool IsCurrentProcessElevated()
    {
      ...
      bool bGranted;
      _.Registry.CheckAccess(HKU, @"S-1-5-19\Environment", Missing.Value, out bGranted);
      return bGranted;
    }
