...
& {
  ...
  dotnet.exe $csc_dll /nologo /target:$($DebugPreference -eq 'Continue' ? 'exe':'winexe') /win32icon:"$PSScriptRoot\menu.ico" /reference:"$BinDir\Interop.IWshRuntimeLibrary.dll" /reference:$(Get-WpfLibrary PresentationFramework) /reference:$(Get-WpfLibrary PresentationCore) /reference:$(Get-WpfLibrary WindowsBase) /reference:$(Get-CoreLibrary System.Xaml) /reference:$(Get-CoreLibrary System.Management) /reference:$(Get-CoreLibrary System.Text.RegularExpressions) /reference:$(Get-CoreLibrary System.Linq) /reference:$(Get-CoreLibrary System.Core) /reference:$(Get-CoreLibrary System) /reference:$(Get-CoreLibrary mscorlib) /reference:$(Get-CoreLibrary Microsoft.CSharp) /out:$(($ConvertExe = "$BinDir\cvmd2html.exe")) "$(($SrcDir = "$PSScriptRoot\src"))\AssemblyInfo.cs" "$SrcDir\ErrorLog.cs" "$SrcDir\Package.cs" "$SrcDir\Parameters.cs" "$PSScriptRoot\Program.cs" "$SrcDir\Setup.cs" "$SrcDir\Util.cs"
